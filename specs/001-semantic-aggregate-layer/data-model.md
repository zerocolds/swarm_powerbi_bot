...
### MultiPlan
План выполнения для одного вопроса пользователя (результат одношагового planning).
```
MultiPlan:
  objective: str             # цель вопроса
  intent: str                # comparison | single | decomposition | trend
  queries: list[AggregateQuery]  # до 10 запросов
  topic: str                 # для chart type
  render_needed: bool
...
aggregates:
  - id: client_outflow
    ...
    returns:
      ...
      - TotalVisits: int
      - TotalSpent: float
    allowed_group_by: [master, service, salon]  # подмножество глобального enum
...
SwarmOrchestrator.handle_question()
  │
  ├──► PlannerAgent.run()
  │     ├── LLMClient.plan_aggregates(question, aggregate-catalog.yaml)
  │     │   → MultiPlan с list[AggregateQuery] (один LLM-вызов, timeout 5s)
  │     └── Fallback: TopicRegistry.detect_topic() → single AggregateQuery
  │         (при LLM timeout/error или невалидном плане; circuit breaker: 3 fails → 60s fallback)
  │
  ├──► MasterResolver.resolve(question, object_id) [если имя в вопросе]
  │     └── SELECT from tbMasters + difflib.SequenceMatcher
  │
  ├──► SQLAgent.run_multi(plan: MultiPlan) [параллельно, asyncio.gather + Semaphore(5)]
  │     ├── AggregateRegistry.validate(query) → whitelist + param check + allowed_group_by
  │     ├── SQLClient.execute_aggregate(aggregate_id, params) [×N, max 10, timeout 10s]
