...
Переход от жёстко привязанных 17 stored procedures (keyword-matching через topic_registry) к семантическому слою агрегатов: LLM выбирает агрегаты из полного каталога в одношаговом planning, SQLAgent выполняет до 10 параллельных запросов к предрассчитанным (materialized) агрегатам, AnalystAgent синтезирует multi-result ответ.

4 фазы: Discovery (dev-time bootstrap: PBIX→YAML→SQL) → SQL layer (materialized агрегаты) → LLM planning (одношаговый) → Сравнения/Композиция. При росте каталога >50 агрегатов — переход на двухшаговый planning (category index → детальный каталог).
...
**Performance Goals**: ≤15 сек на вопрос (до 10 параллельных запросов по 10 сек каждый); materialized агрегаты для снижения латентности; LLM call timeout: 5 сек; circuit breaker: 3 consecutive timeouts → TopicRegistry fallback на 60 сек
**Constraints**: read-only SQL user; LLM никогда не генерирует SQL; whitelist aggregate_id + typed parameters; max 10 queries/question; asyncio.Semaphore(5) для ограничения параллельных SQL-запросов
...
│   ├── planner.py                     # ПЕРЕРАБОТКА: одношаговый planning по полному каталогу
...
├── services/
│   ├── llm_client.py                  # Новый system prompt: полный каталог агрегатов
...
│   ├── aggregate_registry.py          # НОВЫЙ: загрузка каталогов + whitelist + parameter validation + dispatch
...
├── test_aggregate_registry.py         # НОВЫЙ: загрузка каталогов, whitelist, dispatch, типизация параметров
...
**Structure Decision**: Расширение существующей плоской структуры `src/swarm_powerbi_bot/`. Новые модули добавляются в `services/`. catalog_loader и parameter_validator объединены в `aggregate_registry.py` (разделять при >300 строк). Каталоги загружаются при старте, hot-reload не реализуется (YAGNI). Тесты PlannerAgent: mock LLMClient + contract validation JSON→MultiPlan. Чёткое разделение на:
