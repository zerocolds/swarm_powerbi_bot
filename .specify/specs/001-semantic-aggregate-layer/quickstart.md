...
# catalog_loader.py → aggregate_registry.py → parameter_validator.py → master_resolver.py → query_logger.py

# Изменённые модули:
# agents/planner.py (двухшаговый planning)
...
```bash
# Расширение PlannerAgent: intent=comparison → два запроса с разными периодами
