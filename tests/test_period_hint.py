...
def test_bare_month_with_soft_sign_april():
    """Нормативная форма на -ь корректно распознаётся: «апрель» — именительный падеж, не постфикс (round 3: добавлено `|ь` в _RE_MONTH_BARE)."""
    assert has_period_hint("выручка апрель") is True


def test_bare_month_with_soft_sign_january():
    """Нормативная форма на -ь корректно распознаётся: «январь» — именительный падеж, не постфикс (round 3: добавлено `|ь` в _RE_MONTH_BARE)."""
    assert has_period_hint("выручка январь") is True


def test_bare_month_with_soft_sign_september():
    """Нормативная форма на -ь корректно распознаётся: «сентябрь» + год — `ь` это именительный падеж, не постфикс (round 3: добавлено `|ь` в _RE_MONTH_BARE)."""
