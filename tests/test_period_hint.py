...
def test_fr009_baseline_true(question):
    """FR-009 frozen baseline: эти запросы ДОЛЖНЫ давать True, не регрессировать.

    Любое сужение `_RE_MONTH_BARE` или удаление ключевых слов из словаря
    периода (неделя/месяц/квартал/год/полугодие/вчера/сегодня) должно ронять
    эти кейсы. См. specs/012-fix-bare-months/spec.md FR-009.
    """
...
def test_fr009_baseline_false(question):
    """FR-009 frozen baseline: эти запросы ДОЛЖНЫ давать False, не регрессировать.

    Расширение регэкса не должно случайно ловить эти запросы (имена,
    производные слова без периода). См. specs/012-fix-bare-months/spec.md FR-009.
    """
