...
# I-7: flat alternations, no nested quantifiers — ReDoS-safe
_RE_MONTH = re.compile(
    r"за\s+"
    r"(январ(?:ь|я|ю|е|ём)?\b"
    r"|феврал(?:ь|я|ю|е|ём)?\b"
    r"|март(?:а|у|е|ом)?\b"
    r"|апрел(?:ь|я|ю|е|ём)?\b"
    r"|ма[йя]\b"
    r"|июн(?:ь|я|ю|е|ем)?\b"
    r"|июл(?:ь|я|ю|е|ем)?\b"
    r"|август(?:а|у|е|ом)?\b"
    r"|сентябр(?:ь|я|ю|е|ём)?\b"
    r"|октябр(?:ь|я|ю|е|ём)?\b"
    r"|ноябр(?:ь|я|ю|е|ём)?\b"
    r"|декабр(?:ь|я|ю|е|ём)?\b)"
    r"(?:\s+(\d{4}))?",
    re.IGNORECASE,
)
